...
/**
 * Test component to demonstrate the alarm management system on the dashboard
 * This component can be used to test that only one alarm popup appears at a time
 */
const DashboardAlarmTest = () => {
  ...
  const handleSimulateProductionCycle = () => {
    // Simulate typical production cycle with alarms
    const cycleEvents = [
      { delay: 0, type: 'operation-success', message: 'Cycle started: Part loaded' },
      { delay: 1000, type: 'operation-success', message: 'Scanner triggered successfully' },
      { delay: 2000, type: 'operation-success', message: 'Marking operation completed' },
      { delay: 3000, type: 'operation-success', message: 'Cycle completed successfully' },
    ];

    cycleEvents.forEach((event) => {
      setTimeout(() => {
        showAlarm(event.type, event.message, 'normal');
      }, event.delay);
    });
  };

  const handleSimulateErrorScenario = () => {
    // Simulate error scenario with multiple alarms
    const errorEvents = [
      { delay: 0, type: 'part-presence', message: 'Part not detected in fixture' },
      { delay: 2000, type: 'machine-error', message: 'Servo motor communication error' },
      {
        delay: 4000,
        type: 'safety-violation',
        message: 'Safety violation: Door open during operation',
    ...
    errorEvents.forEach((event) => {
      setTimeout(() => {
        showAlarm(event.type, event.message, event.type === 'safety-violation' ? 'high' : 'normal');
      }, event.delay);
    });
  };
  ...
  return (
    <Card className="w-full max-w-4xl bg-gray-800 text-white border-gray-600 mb-4">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            <Button
              onClick={handleSimulateProductionCycle}
              variant="secondary"
            ...
            <Button
              onClick={handleSimulateErrorScenario}
              variant="secondary"
        ...
        <div className="text-xs text-gray-400 space-y-1">
          ...
          <ul className="list-disc list-inside space-y-1 ml-2">
            ...
            <li>Production events should show appropriate notifications</li>
          </ul>
        </div>
      </CardContent>
    </Card>
...
export default DashboardAlarmTest;

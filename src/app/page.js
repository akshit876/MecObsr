...
function Page() {
  ...
  useEffect(() => {
    ...
    // Handle cycle completion - clear OCR input when cycle ends (OK, NG, or NA)
    // Backend sends data sorted by Timestamp descending, so first record is the most recent
    const handleCycleCompletion = (data) => {
      if (data?.data && Array.isArray(data.data) && data.data.length > 0) {
        // Check only the first (most recent) record since data is sorted descending
        const lastRecord = data.data[0];
        if (
          lastRecord?.Result === 'OK' ||
          lastRecord?.Result === 'NG' ||
          lastRecord?.Result === 'NA'
        ) {
          // Clear OCR input when a cycle completes with OK, NG, or NA
    ...
    socket.on('csv-data', handleCycleCompletion);
    ...
    return () => {
      ...
      socket.off('csv-data', handleCycleCompletion);
  ...
  const handleOcrSubmit = () => {
    ...
    if (trimmedOcrData.length < 7) {
      toast.error(
        `Invalid OCR data length. Expected length >= 7, received length: ${trimmedOcrData.length}`,
        {
          position: 'top-right',
          autoClose: 5000,
        },
